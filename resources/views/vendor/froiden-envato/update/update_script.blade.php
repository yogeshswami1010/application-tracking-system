<!-- Use SweetAlert2 instead of the old unpkg SweetAlert (Swal.fire is from sweetalert2) -->

<script type="text/javascript">
(function () {
    'use strict';

    function mountUpdateApplication($) {
        if (window.__recruitUpdateApplicationScriptMounted) {
            return true;
        }
        if (!$ || typeof $.easyAjax !== 'function') {
            return false;
        }
        window.__recruitUpdateApplicationScriptMounted = true;

    var updateAreaDiv = $('#update-area');
    var refreshPercent = 0;
    var checkInstall = true;

    function applySwalUpdateButtonStyles() {
        ...
        if (confirmButton) {
            ...
            confirmButton.style.padding = '0.625rem 1.25rem';
        ...
        if (cancelButton) {
            ...
            cancelButton.style.padding = '0.625rem 1.25rem';
    ...
    $('#update-app').click(function () {
        ...
        Swal.fire({
            ...
            focusConfirm: false,
            customClass: {
                confirmButton: 'text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 font-semibold rounded-lg text-sm px-5 py-2.5 text-center justify-center sm:w-auto',
                denyButton: 'text-white bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 font-semibold rounded-lg text-sm px-5 py-2.5 text-center justify-center sm:w-auto',
                cancelButton: 'text-white bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800 font-semibold rounded-lg text-sm px-5 py-2.5 text-center justify-center sm:w-auto'
            },
            showClass: {
    ...
    window.getPurchaseData = getPurchaseData;
        return true;
    }

    function tryMount() {
        return mountUpdateApplication(window.jQuery || window.$);
    }
    ...
    if (!tryMount()) {
        ...
        var timer = setInterval(function () {
            ...
            if (tryMount() || attempts >= maxAttempts) {
                clearInterval(timer);
                if (attempts >= maxAttempts && (!window.jQuery || typeof window.jQuery.easyAjax !== 'function')) {
                    console.error('Update Application: easyAjax not available. In DevTools → Network, open froiden-helper/helper.js (expect HTTP 200 after jQuery).');
                }
            }
        }, 50);
    }
