...
<div class="rounded-2xl border border-[#DDE6F5] bg-gradient-to-br from-white to-blue-50/50 p-5 shadow-sm">
    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-1 text-[11.5px] text-[#718096]">Creates an application in the selected job's Applied stage and adds the candidate to Candidate Database.</p>
        </div>
        <form method="POST" action="{{ route('admin.consortium-registrations.move-to-job', $registration) }}" class="flex w-full flex-col gap-2 sm:flex-row lg:max-w-2xl" onsubmit="return confirm('Move this candidate to the selected job?')">
            @csrf
            <select name="job_id" required class="min-w-0 flex-1 rounded-xl border border-[#CCD6E5] bg-white px-3 py-2.5 text-[12px] font-medium text-[#334155] outline-none focus:border-blue-500">
            ...
            <button type="submit" class="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-blue-600 px-5 py-2.5 text-[12px] font-bold text-white shadow-sm hover:bg-blue-700">
