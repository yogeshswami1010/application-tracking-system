...
<div class="overflow-hidden rounded-2xl border border-[#E8E6E1] bg-white shadow-sm">
    ...
    <div class="overflow-x-auto"><table class="w-full text-left"><thead class="bg-[#F8F9FB] text-[10px] uppercase tracking-wider text-[#8892A0]"><tr><th class="px-5 py-3"><input type="checkbox" class="cc-all" aria-label="Select all candidates on this page">Candidate</th><th class="px-5 py-3">Contact</th><th class="px-5 py-3">City</th><th class="px-5 py-3">Availability</th><th class="px-5 py-3 text-right">Action</th></tr></thead><tbody>
    @forelse($registrations as $registration)
        <tr class="border-t border-[#F0EEE9] hover:bg-[#FBFCFE]">
            <td class="px-5 py-4"><input type="checkbox" class="cc-select" data-type="registration" value="{{ $registration->id }}" aria-label="Select {{ $registration->first_name }} {{ $registration->last_name }}"><a href="{{ route('admin.consortium-registrations.show', ['registration' => $registration->id, 'from' => 'temp-staffing', 'search' => request('search'), 'page' => request('page')]) }}" class="flex items-center gap-3"><span class="flex h-9 w-9 items-center justify-center rounded-full bg-blue-50 text-[12px] font-bold text-blue-600">{{ strtoupper(substr($registration->first_name,0,1).substr($registration->last_name,0,1)) }}</span><span class="text-[13px] font-semibold text-[#1A1E2E] hover:text-blue-600 hover:underline">{{ $registration->first_name }} {{ $registration->last_name }}</span></a></td>
