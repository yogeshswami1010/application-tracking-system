<div class="job-aux-manage space-y-5">
            ...
            <table class="min-w-full divide-y divide-gray-100 text-left text-[13px]">
                <thead class="sticky top-0 z-[1] bg-[#F8F7F4]">
                    <tr>
                        ...
                        <th class="px-3 py-2.5 font-bold uppercase tracking-wide text-[10.5px] text-[#8892A0]">@lang('modules.jobs.workExperience')</th>
                ...
                <tbody class="divide-y divide-gray-100 bg-white">
                    @forelse($workExperiences as $key => $workExperience)
                        <tr id="dept-{{ $workExperience->id }}" class="transition hover:bg-[#FAFAF8]/80">
                            <td class="px-3 py-2.5 text-[#8892A0]">{{ $key + 1 }}</td>
                            <td class="px-3 py-2.5 font-medium text-[#0F1F3D]" id="deptName{{ $workExperience->id }}">{{ ucwords($workExperience->work_experience) }}</td>
                            <td class="px-3 py-2.5 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    <button type="button" data-dept-id="{{ $workExperience->id }}" class="work-exp-edit-row inline-flex h-8 w-8 items-center justify-center rounded-lg bg-[#2563EB] text-white shadow-sm transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/30" title="@lang('app.edit')" aria-label="@lang('app.edit')">
                                        <i class="fa fa-pencil text-xs" aria-hidden="true"></i>
                                    </button>
                                    <button type="button" data-dept-id="{{ $workExperience->id }}" class="work-exp-delete-row inline-flex h-8 w-8 items-center justify-center rounded-lg bg-red-500 text-white shadow-sm transition hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-500/30" title="@lang('app.delete')" aria-label="@lang('app.delete')">
    ...
    <div class="rounded-xl border border-dashed border-gray-200 bg-[#FAFAF8]/50 p-4">
        <form id="createdepartment" class="ajax-form space-y-4" method="post">
            @csrf
            <div>
                <label for="work_experience_name" class="mb-1.5 block text-[11.5px] font-bold uppercase tracking-wide text-slate-500">@lang('modules.jobs.workExperience')</label>
                <input type="text" name="work_experience" id="work_experience_name" autocomplete="off" class="form-control w-full rounded-xl border border-gray-200 bg-white px-3.5 py-2.5 text-[13px] text-[#0F1F3D] placeholder:text-gray-300 outline-none transition-all focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/10" placeholder="@lang('modules.jobs.workExperience')">
...
<script>
(function () {
    function refreshWorkExperienceSelect(rData) {
        var opts = ['<option value="">—</option>'];
        $.each(rData, function (i, value) {
            opts.push('<option value="' + value.id + '">' + $('<div/>').text(value.work_experience).html() + '</option>');
        });
        var $wx = $('#work_experience');
        if ($wx.data('select2')) {
            $wx.select2('destroy');
        }
        $wx.html(opts.join(''));
    ...
    $('#save-department').on('click', function () {
        $.easyAjax({
            ...
            success: function (response) {
                refreshWorkExperienceSelect(response.data);
                closeModal();
            }
        });
    });
    ...
    $('#update-department').on('click', function () {
        var id = $('#edit_id').val();
        var url = "{{ route('admin.work-experience.update', ':id') }}".replace(':id', id);
        ...
        $.easyAjax({
            ...
            container: '#createdepartment',
            type: 'POST',
            data: { _token: '{{ csrf_token() }}', _method: 'PUT', work_experience: name },
            success: function (response) {
                if (response.status === 'success') {
                    refreshWorkExperienceSelect(response.data);
                    $('#deptName' + id).text(name);
                    $('#edit_id').val('');
                    $('#work_experience_name').val('');
    ...
    $(document).off('click.workExpAux', '.work-exp-edit-row').on('click.workExpAux', '.work-exp-edit-row', function () {
        var id = $(this).data('dept-id');
        $('#edit_id').val(id);
        $('#work_experience_name').val($.trim($('#deptName' + id).text()));
        $('#update-department').prop('hidden', false);
        $('#cancel-edit-department').prop('hidden', false);
        $('#save-department').prop('hidden', true);
    });

    $(document).off('click.workExpAux', '.work-exp-delete-row').on('click.workExpAux', '.work-exp-delete-row', function () {
        var id = $(this).data('dept-id');
        swal({
            ...
            var url = "{{ route('admin.work-experience.destroy', ':id') }}".replace(':id', id);
            $.easyAjax({
                type: 'POST',
                url: url,
