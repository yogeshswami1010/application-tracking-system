<div class="flex items-center justify-between p-4 border-b border-gray-200">
    <h4 class="text-lg font-semibold text-gray-900"><i class="icon-plus"></i> @lang('app.designation')</h4>
...
<div class="p-4">
    <div class="overflow-x-auto mb-4">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">@lang('app.designation')</th>
            ...
            @forelse($designations as $key=>$designation)
                <tr id="desig-{{ $designation->id }}">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap" id="desgName{{ $designation->id }}">{{ ucwords($designation->name) }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <a href="javascript:;" data-dept-id="{{ $designation->id }}" class="btn btn-sm btn-danger delete-designation"><i class="fa fa-times"></i></a>
                        <a href="javascript:;" data-dept-id="{{ $designation->id }}" class="btn btn-sm btn-info edit-designation"><i class="fa fa-pencil"></i> </a>
                    </td>
                </tr>
            @empty
                <tr>
    ...
    <hr class="my-4">
    <form id="createDesignation" class="ajax-form space-y-4" method="post">
        {{ csrf_field() }}
        <div class="form-body">
        ...
        <div class="form-actions flex items-center gap-2">
            ...
            <button type="button" id="update-designation" class="btn btn-success hidden"> <i class="fa fa-check"></i> @lang('app.update')</button>
        </div>
    </form>
...
<script>
        ...
        $.easyAjax({
            ...
            success: function (response) {
                if(response.status == 'success'){
                    ...
                    if ($('#designation').hasClass('select2')) {
                        $('#designation').select2();
                    }
        ...
        $.easyAjax({
            ...
            success: function (response) {
                if(response.status == 'success'){
                    ...
                    $('#update-designation').addClass('hidden');
                    $('#save-designation').removeClass('hidden');
                }
            }
        })
    })
    ...
    $('body').on('click', '.edit-designation', function(){
        ...
        $('#update-designation').removeClass('hidden');
        $('#save-designation').addClass('hidden');
    });
