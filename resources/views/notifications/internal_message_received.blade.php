@php
    ...
    $conversationUrl = route('admin.internal-messages.index', ['recipient' => $senderId]);
@endphp

<a href="javascript:;"
   data-link="{{ $conversationUrl }}"
   class="read-notification block"
   data-notification-id="{{ $notification->id }}"
   data-internal-notification-id="{{ $notification->id }}">
    <div class="flex items-start gap-3 border-b border-[#F0EEE9] px-4 py-3 transition hover:bg-[#F8F7F4]">
        <div style="width:32px;height:32px;border-radius:50%;background:#EFF6FF;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
            <i class="fa fa-comments-o" style="color:#2563EB;font-size:13px;"></i>
        </div>
        <div style="flex:1;min-width:0;">
            <div style="font-size:12.5px;font-weight:600;color:#1A1E2E;margin-bottom:2px;">
                {{ $notification->data['sender_name'] ?? 'A team member' }} sent you a message
            </div>
            <div style="font-size:12px;color:#5A6478;line-height:1.5;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;max-width:220px;">
                “{{ $notification->data['message_text'] ?? '' }}”
            </div>
