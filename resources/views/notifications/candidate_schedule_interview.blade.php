<a href="javascript:;" data-link ="{{ route('admin.job-applications.index') }}" class="block px-4 py-2 text-sm hover:bg-gray-100 read-notification" data-notification-id="{{ $notification->id }}">
    <div class="flex items-center justify-between">
        <div class="flex items-center flex-1 min-w-0">
            ...
            <span class="text-truncate-notify truncate" style="overflow-y: hidden" title="full name">{{ __('messages.notifications.candidateScheduleInterview'), ['name' => $notification->data['data']['full_name']]) }}</span>
        </div>
