(function ($) {
    ...
    $.easyAjax = function (options) {
            ...
            opt.beforeSend = function () {
                ...
                $(opt.container)
                    ...
                    .each(function () {
                        ...
                        $(this).removeClass('has-error border-red-500');
        ...
        function showResponseMessage(msg, type, toastrOptions) {
            ...
            if (opt.messagePosition == 'toastr') {
                ...
                var html = '<div class="p-4 rounded-md border ' + tailwindClasses[type] + '">' + msg + '</div>';
                if (ele.length == 0) {
                    $(opt.container)
                        .find('.form-group:first')
                        .before('<div id="alert" class="mb-4">' + html + '</div>');
                } else {
                    ele.html(html);
                }
            }
        ...
        function handleFail(response) {
            ...
            if (typeof response.errors != 'undefined') {
                ...
                $(opt.container).find('.has-error').removeClass('has-error border-red-500');
                $(opt.container).find('.has-error input, .has-error select, .has-error textarea').removeClass('border-red-500');
                ...
                if (opt.errorPosition == 'field') {
                    for (var i = 0; i < keys.length; i++) {
                        ...
                        helpBlockContainer.append(
                            '<div class="help-block text-red-600 text-sm mt-1">' + response.errors[keys[i]] + '</div>'
                        );
                    ...
                    var html = '<div class="p-4 rounded-md border bg-red-50 text-red-800 border-red-200">' + errorMsg + '</div>';
    ...
    $.ajaxModal = function (selector, url, onLoad) {
        ...
        console.log('Opening modal:', selector, 'URL:', url);
        $modal.removeClass('hidden');

        // Outer shell stays block/fixed; inner wrapper uses Tailwind flex for viewport centering.
        // Do not set display:flex here — it breaks .flex.min-h-full centering on the child.
        $modal.css({
            display: '',
            visibility: 'visible',
            opacity: '1',
        ...
        // Find the modal content area - try multiple selectors in order of specificity
        var $modalContent = null;

        // Prefer explicit shell (interview schedule & other redesigned modals)
        $modalContent = $modal.find('.modal-data-shell div.p-6.overflow-y-auto').first();

        // Try: div with both p-6 and overflow-y-auto classes inside #modal-data-application (most specific)
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('#modal-data-application div.p-6.overflow-y-auto').first();
        }

        // Try: div with both p-6 and overflow-y-auto classes anywhere in modal
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('div.p-6.overflow-y-auto').first();
        }

        // Try: any element with p-6 class that also has overflow-y-auto
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('.p-6[class*="overflow-y-auto"]').first();
        }

        // Try: second div inside #modal-data-application (usually the content area after header)
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('#modal-data-application > div').eq(1);
        }

        // Try: div with p-6 class inside #modal-data-application
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('#modal-data-application div.p-6').first();
        }

        // Try: div with p-6 class anywhere
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('div.p-6').first();
        }

        // Try: any element with overflow-y-auto
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('[class*="overflow-y-auto"]').first();
        }

        // Try: modal-body class
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('.modal-body').first();
        }

        // Last resort: find any div that's not the backdrop or header
        if ($modalContent.length === 0) {
            $modalContent = $modal.find('> div > div').not('.fixed').not('[class*="flex items-center justify-between"]').first();
        }
        ...
        if ($modalContent.length > 0) {
            ...
            console.warn('Modal content area not found for selector:', selector, 'Modal structure:', $modal[0]);
        }
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (typeof response === 'string') {
                    // Direct HTML string (most common case)
                    content = response;
                } else if (response && response.view) {
                    // Laravel response with view property
                    content = response.view;
                } else if (response && response.html) {
                    // Response with html property
                    content = response.html;
                } else if (response && response.status === 'success' && response.view) {
                    // Success response with view
                    content = response.view;
                } else if (response && typeof response === 'object') {
                    // Try to extract HTML from object
                    console.warn('Unexpected response format:', response);
                    content = response.view || response.html || JSON.stringify(response);
                } else {
                    content = response || '';
                }
                ...
                if (content) {
                    console.log('Content received, length:', content.length, 'Content area found:', $modalContent.length > 0);
                    if ($modalContent.length > 0) {
                        $modalContent.html(content);
                        console.log('Content inserted into modal');
                    } else {
                        // Try to find content area one more time
                        $modalContent = $modal.find('div.p-6, div[class*="overflow"], #modal-data-application > div').eq(1);
                        if ($modalContent.length > 0) {
                            $modalContent.html(content);
                            console.log('Content inserted using fallback selector');
                        } else {
                            // Last resort: replace all content in modal except backdrop
                            $modal.find('> div > div').not('.fixed').not('[onclick]').first().html(content);
                            console.warn('Used fallback method to insert content into modal');
                        }
                    }
                } else {
                    console.error('No content to display in modal. Response:', response);
                }
            ...
            error: function (xhr, status, error) {
                console.error('Error loading modal content:', error, xhr);
                var errorMsg =
                    '<div class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded">Error loading content. Please try again.</div>';
                if ($modalContent && $modalContent.length > 0) {
                    $modalContent.html(errorMsg);
                } else {
                    // Try to show error in modal
                    $modal.find('div').not('.fixed').first().html(errorMsg);
                }
            },
        });
    };
...
!(function (a) {
    a(['jquery'], function (a) {
        return (function () {
            ...
            function l(b) {
                return (
                    (r = a('<div/>').attr('id', b.containerId).addClass(b.positionClass).attr('aria-live', 'polite').attr('role', 'alert')),
            ...
            function o(b) {
                ...
                'undefined' != typeof b.optionsOverride && ((g = a.extend(g, b.optionsOverride)), (h = b.optionsOverride.iconClass || h)),
                    t++,
                    (r = c(g, !0));
                var i = null,
                    j = a('<div/>'),
                ...
                return (
                    ...
                    g.closeButton &&
                        ...
                        m.click(function (a) {
                            a.stopPropagation
                                ? a.stopPropagation()
                                : void 0 !== a.cancelBubble && a.cancelBubble !== !0 && (a.cancelBubble = !0),
                                d(!0);
                        }),
                    g.onclick &&
                        j.click(function () {
            ...
            var r,
                s,
                t = 0,
                u = { error: 'error', info: 'info', success: 'success', warning: 'warning' },
                v = {
                    clear: h,
                    remove: i,
                    error: b,
